...
export const reviewsAPI = {
  createReview: async (reviewData) => {
    try {
      const response = await expressClient.post(API_CONFIG.ENDPOINTS.REVIEWS.BASE, reviewData);
      return response.data;
    } catch (error) {
...
export const calculateRouteWithTracking = async (points, trackerId = null) => {
  try {
    ...
    // Make request to Python backend
    const response = await pythonClient.post('/route', { points });
    const data = response.data;
...
export const getModelChatWithTracking = async (query, options = {}, trackerId = null) => {
  try {
    ...
    // Make request to Python backend
    const response = await pythonClient.post('/model/chat', requestData);
    const data = response.data;
...
export const createTrackerWithAIData = async (tripId, email, travelerName = '', phone = '', aiRecommendationData = null) => {
  try {
...
export const api = {
  ...
  pythonBackend: {
    getRecommendations: (query, options) => pythonClient.post('/recommend', { query, ...options }),
    geocode: (query) => pythonClient.get('/geocode', { params: { q: query } }),
    calculateRoute: (points) => pythonClient.post('/route', { points }),
    getModelChat: (query, options) => pythonClient.post('/model/chat', { query, ...options }),
    getHealth: () => pythonClient.get('/health'),
    getCities: () => pythonClient.get('/cities'),
    getCategories: () => pythonClient.get('/categories')
  },
